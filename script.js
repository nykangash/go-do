...
function applyTheme(theme) {
    ...
    themeToggleIndicator.style.transform = theme === 'light' ? 'translateX(0.15rem)' : 'translateX(1.6rem)';
...
completedTasksBtn.addEventListener('click', () => alert('Completed tasks archive coming soon!'));
        ...
        function createColumnElement(cData) {
            ...
            m.title = "Options";
            // *** MODIFIED: Ensure 'e' is passed to showColumnContextMenu ***
            ...
            btn.className = 'w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-3 rounded-md text-sm';
        ...
        function sortColumn(id, type) {
            ...
            else if (type === 'date-new') c.tasks.sort((a, b) => b.createdAt - a.createdAt);
            renderBoard();
        }
        ...
        function clearCompleted(id) {
            ...
            const c = b ? b.columns.find(col => col.id === id) : null;
            if (!c) return;
            c.tasks = c.tasks.filter(t => !t.subTasks.every(st => st.completed));
            renderBoard();
        }
        ...
        function toggleSubtaskVisibility(cId, tId) {
            const el = document.querySelector(`.task-item[data-task-id="${tId}"]`);
            if (!el) return;
            const f = el.querySelector('.add-subtask-form');
            const l = el.querySelector('.subtask-list-container');
            if (!f || !l) return;
            const hidden = f.style.display === 'none';
            f.style.display = hidden ? 'flex' : 'none';
            l.style.display = hidden || l.children.length > 0 ? 'flex' : 'none';
            if (hidden && f.querySelector('input')) f.querySelector('input').focus();
        }
        ...
        function createSubTaskElement(stData, tId, cId, pColor) {
            ...
            chk.addEventListener('change', () => toggleSubTaskComplete(cId, tId, stData.id));
            ...
            span.style.color = getContrastingTextColor(pColor); // Sub-task text with contrast.
        ...
        function hideAllMenus() {
            ...
            if (currentOpenColumnElement) {
                ...
                if (columnContextMenu.parentNode !== body) {
                    body.appendChild(columnContextMenu);
                }
                currentOpenColumnElement = null;
            }
            // Ensure task menu is also back in body
        ...
        function populateAndShowContextMenu(x, y, tData) {
            ...
            PRESET_COLORS.forEach(c => {
                ...
                if (c.value === tData.color) d.style.fontWeight = 'bold';
                d.prepend(s);
                d.addEventListener('click', () => {
        ...
        // Populates and shows the context menu for a column. (MODIFIED)
        function showColumnContextMenu(btnEl, cId) {
            ...
            columnEl.style.position = 'relative'; // Make it the positioning context
            ...
            items.forEach((i, idx) => {
                ...
                if (i.submenu) {
                    ...
                    sub.className = 'menu-base submenu-content';
                    i.submenu.forEach(col => {
                        const subItem = document.createElement('div');
                        subItem.className = 'menu-item';
                        const s = document.createElement('div');
                        s.className = 'context-menu-color-swatch';
                        s.style.backgroundColor = col.value;
                        subItem.innerHTML = `<span>${col.name}</span>`;
                        subItem.prepend(s);
                        subItem.addEventListener('click', e => {
                            e.stopPropagation();
                            updateColumnColor(cId, col.value);
                            hideAllMenus();
                        });
            ...
            const menuX = btnEl.offsetLeft; // X relative to column
            const menuY = btnEl.offsetTop + btnEl.offsetHeight + 5; // Y relative to column (below button)
        ...
        function handleTaskDragEnterColumn(e) {
            ...
            if (draggedTask && col) col.classList.add('drag-over');
        }
        // Removes the visual indicator when a task is dragged out of a column.
        function handleTaskDragLeaveColumn(e) {
            const col = e.target.closest('.column');
            if (draggedTask && col && !col.contains(e.relatedTarget)) col.classList.remove('drag-over');
        }
        ...
        function handleTaskDrop(e, tColId) {
            ...
            if (tColEl) tColEl.classList.remove('drag-over');

            const b = getCurrentBoard();
            if (!b) return;
            const tId = draggedTask.dataset.taskId;
            const sCol = b.columns.find(c => c.id === sourceColumnId);
            const sIdx = sCol ? sCol.tasks.findIndex(t => t.id === tId) : -1;
            if (sIdx === -1) return; // Find the task in the source column.
            const [tData] = sCol.tasks.splice(sIdx, 1); // Remove from source.
            const tCol = b.columns.find(c => c.id === tColId);
            if (!tCol) {
                sCol.tasks.splice(sIdx, 0, tData);
                renderBoard();
                return;
            } // Find the target column.
            // Find the position to insert the task based on mouse Y-coordinate.
            const tList = tColEl.querySelector('.task-list');
            const refNode = Array.from(tList.querySelectorAll('.task-item:not(.dragging)')).find(el => e.clientY < el.getBoundingClientRect().top + el.offsetHeight / 2);
            const refIdx = refNode ? tCol.tasks.findIndex(t => t.id === refNode.dataset.taskId) : -1;
            if (refIdx !== -1) tCol.tasks.splice(refIdx, 0, tData);
            else tCol.tasks.push(tData); // Insert or append to target.
        ...
        // --- Global Listeners ---
        // Handles clicks outside menus or sidebar to close them. (MODIFIED)
        document.addEventListener('click', (e) => {
            if (!taskColorContextMenu.contains(e.target)) {
                taskColorContextMenu.style.display = 'none';
            }
            // If the click is NOT inside the column menu AND NOT the 3-dot button (or its icon)
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Ensure menus are in the body initially
            body.appendChild(taskColorContextMenu);
            body.appendChild(columnContextMenu);
        });
